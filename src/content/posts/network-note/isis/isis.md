...
- **L1路由器** 只能和同一区域的L1或L1/L2路由器建立邻居
- **L2路由器** 可以和任何L2或L1/L2路由器建立邻居（不要求区域相同）
...
IS-IS的报文直接封装在二层的，PDU（Protocol Data Unit）分为`IIH`、`LSP`、`SNP`三大类：

| 报文类型 | 全称 | 功能 |
|---------|------|------|
| **IIH（L1/L2）** | IS-IS Hello PDU | 建立和维护邻居关系，分L1 IIH和L2 IIH |
...
> 相比OSPF的8个状态，IS-IS的邻居状态机简单多了。这也是很多人觉得IS-IS更稳定的原因之一。
...
- **可抢占**：新的高优先级路由器可以立即成为DIS
- **没有备份DIS**：不像OSPF有BDR，IS-IS只有DIS，DIS挂了会重新选举

## 5.2 伪节点（Pseudonode）

DIS会产生一个 **伪节点LSP** 来代表这个广播网络：

- 伪节点的LSP ID格式：`DIS的System ID.00-XX`，XX是伪节点编号
- 所有路由器只需要和伪节点建立邻接关系
...
:::tip
**IS-IS默认所有接口cost=10，不区分带宽**，这点新手很容易踩坑。如果忘了手动配置或者开自动计算，千兆口和万兆口在IS-IS看来是一样的。
:::
...
| TLV类型 | 说明 |
|---------|------|
| 128/130 | IP Internal/External Reachability，IPv4路由信息 |
| 132 | IP Interface Address，接口IP地址 |
| 135 | Extended IP Reachability（Wide Metric），扩展IP可达性 |
| 236 | IPv6 Reachability，IPv6路由 |
| 232 | IPv6 Interface Address，IPv6接口地址 |
| 137 | Hostname，主机名映射 |
| 211 | M-ISIS，Multi-Topology（多拓扑） |

:::tip
**IS-IS支持IPv6不需要跑一个新的协议进程**，只需要在原来的IS-IS进程里加IPv6的TLV就行。这也是运营商喜欢IS-IS的原因之一，双栈部署特别方便。
:::
...
IS-IS这个协议虽然学起来感觉比OSPF抽象一点（NSAP地址、TLV这些），但真用在大规模网络里反而更省心。几点体会：

1. **不依赖IP**，链路层就能跑通，基础承载能力强
2. **两级区域结构简单**，L1/L2的分工清晰，不会像OSPF那样搞出各种特殊区域
3. **TLV扩展机制好**，加功能不需要改协议框架，IPv6支持就是最好例子
4. **DIS可抢占**虽然看起来不如OSPF稳定，但实际工程中反而更灵活
5. **默认cost策略坑多**，所有接口cost=10这个默认值一定要改

运营商骨干网选型IS-IS而不是OSPF，基本就是看中它的扩展性和稳定性。企业网可能OSFP更常见，但如果网络规模上去了或者需要做双栈承载，IS-IS确实是更好的选择。
