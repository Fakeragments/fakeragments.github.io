...
> 为什么叫IPv4
**IPv0-IPv3**：1970 年代 DARPA 项目中的实验版本，主要用于 ARPANET 的早期研究，功能不完善，没有大规模部署。
**IPv4**：1981 年正式标准化（RFC 791），是第一个功能完整、设计成熟的版本。
**IPv5**：实际上存在，它被称为 ST（Internet Stream Protocol），用于实验性的实时音视频流传输，但从未成为主流。因为"5"已被占用，下一代协议只能叫 IPv6。

IPv4设计目的就是为了`解决在异构网络之间实现可靠的数据包路由`
- 32位地址范围，在设计之初是非常充裕的，且考虑到当时设备的处理性能
- 无链接，尽力而为，将可靠性依托上层
- 只在意路由寻址

IPv4在1990年代随着万维网爆发式增长，形成了巨大的路径依赖。
 
IPv4 的局限性（主要是`地址耗尽`）通过NAT（网络地址转换）、CIDR（无类别域间路由）、DHCP 等技术缓解。
